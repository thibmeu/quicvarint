...
describe('Round-trip properties', () => {
    ...
    test.prop([arbValidLengthForInt])(
        'decode(encode(n, len)) === n for all valid lengths',
        ({ n, len }) => {
...
describe('Boundary rejection', () => {
    test.prop([fc.integer({ min: MAX + 1, max: MAX + 10_000 })])(
        'encode rejects values above MAX',
        (n) => {
...
describe('Decode never silently corrupts', () => {
    test.prop([fc.uint8Array({ minLength: 1, maxLength: 8 })])(
        'result is in range or throws',
        (bytes) => {
            try {
